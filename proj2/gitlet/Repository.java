...
public class Repository {
    ...
    static void init() {
        ...
        writeContents(HEAD_FILE, "master"); // 更新 master 到 HEAD
    }
    ...
    public static void remove(String fileName) {
        ...
        // 区别是下面的会把 removedfiles 添加，上面的不会
        if (trackedInHead) {
    ...
    public void status() {
        ...
        for (String file : allFiles) {
            if (!cwdFiles.contains(file)) {
                ...
                boolean hasHead = !hBlobId.isEmpty();
                boolean hasStage = !sBlobId.isEmpty();

                if ((hasHead && !hasStage && !blobId.equals(hBlobId))
                        || (hasStage && !blobId.equals(sBlobId))) {
     ...
     */
    private void mergeWithLCA(String splitPoint, String targetCommitId,
                              String currCommitId, String curBranch, String targetBranch) {
        ...
        for (String file : allFiles) {
            ...
            // 3.两分支相同修改：保持不变
            else if (inSplit && inCurr && inTarget &&
                    !splitBlobId.equals(currBlobId) &&
                    !splitBlobId.equals(targetBlobId) &&
                    currBlobId.equals(targetBlobId)) {
                continue;
            }
            // 4.仅当前分支有新文件：保持不变
            ...
            // 8.两分支不同修改（包括删除）：生成冲突文件
                    ...
                    !Objects.equals(splitBlobId, currBlobId) &&
                    !Objects.equals(splitBlobId, targetBlobId) &&
                    !Objects.equals(currBlobId, targetBlobId)) {
                hasConflict = true;
        ...
        Commit mergedCommit = new Commit(mergeMessage, mergedCommits,
                readObject(STAGING_FILE, Stage.class));
